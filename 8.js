const input = `oui inc 550 if p <= 3
abh inc -189 if g == 0
kq inc 164 if znl != 7
...
const result = input
  ...
  .reduce((result, current) => {
    ...
    if (typeof(total[parts[0]]) == 'undefined')
      total[parts[0]] = 0

    if (typeof(total[parts[4]]) == 'undefined')
      total[parts[4]] = 0

    const command = `if (total['${parts[4]}'] ${parts[5]} ${parts[6]}) total['${parts[0]}'] ${parts[1] === 'inc' ? '+=' : '-='} ${parts[2]}`
    eval(command)

    result.total = total

    const maxKey = Object.keys(total).reduce((max, c) => total[max] > total[c] ? max : c)
    const totalMax = total[maxKey]
    result.max = totalMax > result.max ? totalMax : result.max

    return result
  }, { 'total': {}, 'max': 0 })

const maxKey = Object.keys(result.total).reduce((max, current) => result.total[max] > result.total[current] ? max : current)

const highestValue = result.total[maxKey]
highestValue
