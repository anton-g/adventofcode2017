...
const data = input
  ...
  .map((p, idx, arr) => {
    const re = /(\w*) \((\d*)\)(?: -> )*(.*)/
    const parts = re.exec(p)
    
    return {
      'name': parts[1],
      'weight': parseInt(parts[2]),
      'parent': undefined,
      'children': parts[3].length < 1 ? null : parts[3].split(', ')
    }
  })
  .map((p, idx, arr) => {
    if (p.children) {
      p.children = p.children.map(child => {
        const childIdx = arr.findIndex(c => c.name === child)
        arr[childIdx].parent = idx
...
const findRoot = arr => {
  const recursiveFindRoot = (treeArr, node) => {
    if (node.parent) {
      const newNode = treeArr[node.parent]
      return recursiveFindRoot(treeArr, newNode)
    }

    return node
  }

  return recursiveFindRoot(arr, arr[0])
}

const recursiveCalculateWeight = (arr, node) => {
  if (node.children) {
    return node.children
      .map(i => arr[i])
      .reduce((total, current) => total + recursiveCalculateWeight(arr, current), node.weight)
  }

  return node.weight
}

const unbalancedCheck = (arr, node) => {
  var weights = node.children
    .map(i => arr[i])
    .map(child => recursiveCalculateWeight(arr, child))

  return weights.reduce((total, curr) => total + curr, 0) / weights.length === weights[0]
}

const findUnbalanced = (arr, node) => {
  if (!node.children) return null
  
  if (!unbalancedCheck(arr, node)) {
    const childNodes = node.children.map(i => arr[i])
    for (let i = 0; i < childNodes.length; i++) {
      const unbalanced = findUnbalanced(arr, childNodes[i])
      if (unbalanced)
        return unbalanced
    }
...
const calculateBalanceDiff = (arr, node) => {
  console.log(node)
  const weights = new Set(node.children
    .map(i => arr[i])
    .map(c => recursiveCalculateWeight(arr, c)))

  return Math.max(...weights) - Math.min(...weights)
}

const findBadWeightChild = (arr, node) => {
  const children = node.children
    .map(i => arr[i])

  const childWeights = children
    .map(child => recursiveCalculateWeight(arr, child))
  
  for (let i = 0; i < childWeights.length; i++) {
    const weight = childWeights[i];
    const occurences = childWeights.filter(val => val === weight).length

    if (occurences === 1) return children[i]
  }
}

const root = findRoot(data)
const unbalanced = findUnbalanced(data, root)
const diff = calculateBalanceDiff(data, unbalanced)
const child = findBadWeightChild(data, unbalanced)
const result = child.weight - diff
result